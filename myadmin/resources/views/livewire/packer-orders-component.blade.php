                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex gap-2">
                                                        <button class="btn btn-sm btn-primary" wire:click="startPacking({{ $order['id'] }})">
                                                            Pack
                                                        </button>
                                                    </div>
                                                </td>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Pack Order #{{ $selectedOrder->id }}</h5>
                                    ...
                                    <tbody>
                                        @foreach($selectedOrder->items_with_details as $item)
                                            <tr>
                                                ...
                                                <td>{{ $item['quantity'] }}</td>
                                                <td>
                                                    {{ isset($packedItems[$item['product_id']]) 
                                                        ? $packedItems[$item['product_id']]['packed_qty'] 
                                                        : 0 
                                                    }}
                                                </td>
                                                <td>
                                                    @if(isset($packedItems[$item['product_id']]) && 
                                                        $packedItems[$item['product_id']]['packed_qty'] >= $item['quantity'])
        ...
        if (input) {
            input.addEventListener('keydown', function(e) {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    const componentId = document.querySelector('[wire\\:id]').getAttribute('wire:id');
                    Livewire.find(componentId).set('scannedBarcode', this.value);
                    Livewire.find(componentId).scanItem();
                    this.value = '';
                }
            });
